...
export default function VoiceAgent({ onClearKey }: VoiceAgentProps) {
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      ...
      if (!sessionRes.ok) {
        ...
        const msg = err.error ?? "Failed to create session";
        if (sessionRes.status === 401 && onClearKey) {
          onClearKey();
  ...
  }, [handleDataChannelMessage, onClearKey]);
